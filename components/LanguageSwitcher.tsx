...
const LanguageSwitcher: React.FC = () => {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 px-2 py-1 rounded-full bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-400"
    ...
    >
      ...
      <div className="flex items-center w-16 h-7 rounded-full bg-gray-300 dark:bg-gray-600 relative">
        ...
        <span
          className={`absolute top-1/2 -translate-y-1/2 left-0 transition-all duration-200 w-1/2 h-5 rounded-full bg-yellow-400 ${language === 'ar' ? 'translate-x-0' : 'translate-x-full'}`}
          style={{ zIndex: 1 }}
        />
      </div>
    </button>
